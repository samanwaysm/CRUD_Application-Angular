...
export const userReducer = createReducer(
    initialState,
    on(UserActions.login, (state: UserState) => ({ ...state, isLoading: true, error: null })),
    on(UserActions.loginSuccess, (state: UserState, action: { user: UserData, token: string }) => ({
        ...state, user: action.user, isLoading: false, error: null
    })),
    on(UserActions.loginFailure, (state: UserState, action: { error: any }) => ({
        ...state, isLoading: false, error: action.error
    })),

    on(UserActions.getUserData, (state: UserState) => ({ ...state, isLoading: true, error: null })),
